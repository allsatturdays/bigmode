...
func _load_audio_options() -> void:
	var master_volume: float = AudioManager.audio_config.get_master_volume()
	var music_volume: float = AudioManager.audio_config.get_music_volume()
	var sfx_volume: float = AudioManager.audio_config.get_sfx_volume()
	var audio_enabled: bool = AudioManager.audio_config.get_audio_enabled()
...
func _action_master_menu_slider(value: float) -> void:
	AudioManager.audio_config.set_master_volume(value)


func _action_music_menu_slider(value: float) -> void:
	AudioManager.audio_config.set_music_volume(value)


func _action_sfx_menu_slider(value: float) -> void:
	AudioManager.audio_config.set_sfx_volume(value)


func _action_audio_menu_toggle(enabled: bool) -> void:
	AudioManager.audio_config.set_audio_enabled(enabled)
...
func _action_reset_menu_button() -> void:
	...
	AudioManager.audio_config.reset()
	_load_audio_options()


func _connect_signals() -> void:
	GameEvents.menu_slider_value_changed.connect(_on_menu_slider_value_changed)
	GameEvents.menu_toggle_value_changed.connect(_on_menu_toggle_value_changed)
